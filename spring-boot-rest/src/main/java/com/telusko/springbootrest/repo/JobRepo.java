...
//		return jobs.stream().filter(t->t.getPostId()==i).findFirst().get();
	...
	public void updateJob(JobPost jobPost) {
		// TODO Auto-generated method stub
		JobPost jobPost2 = jobs.stream().filter(job-> job.getPostId()==jobPost.getPostId()).distinct().findFirst().get();
	...
	public void deleteJob(int post) {
		// TODO Auto-generated method stub
		jobs.remove(jobs.stream().filter(job->job.getPostId()==post).findFirst().get());
	}
